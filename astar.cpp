...
State move_Al(State& s, int x, int y){
	...
	//Al father state position
	n.father_x_pos = s.al_x_pos;
	n.father_y_pos = s.al_y_pos;
...
void move_rock(State& s){
	...
	if(s.father_x_pos < s.al_x_pos){
		new_rock = make_tuple(s.al_x_pos + 1, s.al_y_pos);
	} else if(s.father_x_pos > s.al_x_pos){
		new_rock = make_tuple(s.al_x_pos - 1, s.al_y_pos);
	} else if(s.father_y_pos < s.al_y_pos){
...
char check_position(State& s, int x, int y, vector<tuple<int, int>> w, vector<tuple<int, int>> sn){
	...
	if((find(w.begin(), w.end(), make_tuple(x,y)) != w.end()) ||
		...
		((x == s.father_x_pos) && (y == s.father_y_pos))){
...
int main(int argc, char *argv[]) {
	...
	if(completed){
		...
		cout << "Path: " << endl;
		while((count < close.size()) && (!flag)){
			father = close.at(count);
			if((state.father_x_pos == -1) && (state.father_y_pos == -1)){
				cout << "(" << state.al_x_pos << ", " << state.al_y_pos << ")" << endl;
				flag = true;
			}else if((father.al_x_pos == state.father_x_pos) && (father.al_y_pos == state.father_y_pos)){
				cout << "(" << state.al_x_pos << ", " << state.al_y_pos << ")" << endl;
				state = father;
				count = 0;
			} else {
