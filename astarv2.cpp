...
State compare_open_close(vector<State>& o, vector<State>& c, bool& completed){
	bool found = false;
	vector<State>::iterator it;
	State state;
	int count = 0;
	//cout << "Open size: " << o.size() << endl;
	//cout << "Close size: " << c.size() << endl;
	if(c.empty()){
		state = o.at(0);
		c.push_back(state);
		o.pop_back();
		...
		for(int i = 0; (i < o.size()) && (!found); i++){
			...
			//cout << "i: " << i << endl;
			//cout << "open size: " << o.size() << endl;
			//cout << "Comparing state 1: " << endl;
			//cout << "(" << state.al_x_pos << ", " << state.al_y_pos << ")" << endl;		
				//cout << "Comparing state 2: " << endl;
				//cout << "(" << c.at(j).al_x_pos << ", " << c.at(j).al_y_pos << ")" << endl;
			//cout << "Rocks: " << state.state_rocks.size() << endl;
			if(find(c.begin(), c.end(), state) == c.end()){
...
void apply_movement(vector<State>& open, State& state, vector<tuple<int, int>>& walls, 
					vector<tuple<int, int>>& snakes, tuple<int, int> goal, int heuristic_function,
					int rows, int cols, int x, int y, int& global_id){
	char c;
	State child_state;

	c = check_position(state, state.al_x_pos + x, state.al_y_pos + y, walls, snakes, goal);
	if((c != 'F') && care_with_snakes(state, snakes)){
		child_state = move_Al(state, state.al_x_pos + x, state.al_y_pos + y);
		if(c == 'R'){
			//cout << "Roca detectada" << endl;
			if((check_position(child_state, child_state.al_x_pos + x, child_state.al_y_pos + y, walls, snakes, goal) == ' ') &&
...
int main(int argc, char *argv[]) {
	...
	while(!open.empty() && !completed){
		...
		*/
		state = compare_open_close(open, close, completed);
		/*
		cout << "Close: ";
		for(State& s : close){
			cout << "(" << s.al_x_pos << ", " << s.al_y_pos << ")" << endl;
		...
		*/
		if(isGoal(state, goal)){
			...
			if(state.al_x_pos + 1 < rows){
				apply_movement(open, state, walls, snakes, goal, heuristic_function,
